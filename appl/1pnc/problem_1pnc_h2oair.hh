...
// Set the fluid system: H2O as phase and component, Air as component
template<class TypeTag>
struct FluidSystem<TypeTag, TTag::PNMOnePTwoCProblem>
{
    //using Policy = FluidSystems::H2OAirDefaultPolicy</*simple*/true>;
    //using H2OAir = FluidSystems::H2OAir<GetPropType<TypeTag, Properties::Scalar>, Policy>;
    using H2OAir = FluidSystems::H2OAir<GetPropType<TypeTag, Properties::Scalar>>;
    static constexpr int phaseIdx = H2OAir::liquidPhaseIdx; 
    using type = FluidSystems::OnePAdapter<H2OAir, phaseIdx>; //switch index of H2O and Air such that H2O is phase and Air the component
};
...
public:
    ...
    : ParentType(gridGeometry, spatialParams)
    {
        ...
        std::cout << "trans Comp Air index:" << transportCompAirIdx << std::endl;
        std::cout << "conti eq index for Air:" << contiAirEqIdx << std::endl;
        std::cout << "conti eq index for H2O:" << contiH2OEqIdx << std::endl;
        std::cout << "transport eq index for Air:" << transportAirEqIdx << std::endl;
    ...
    BoundaryTypes boundaryTypes(const Element& element, const SubControlVolume& scv) const
    {
        ...
        if (onLeftBoundary_(scv) || onRightBoundary_(scv) || onBottomBoundary_(scv))
        {
            ...
            bcTypes.setNeumann(AirIdx); //set mole fraction of Air constant on specified boundaries
                               ...
                               const SubControlVolume& scv) const
    {
        ...
        if(isInletPore_(scv)){
            ...
            values[transportAirEqIdx] = inletMoleFraction_;
            std::cout << "Actual.AirFlux value: " << values[transportAirEqIdx] << "moles"<< std::endl;
        ...
        */
        if(isOutletPore_(scv)){
            ...
            values[transportAirEqIdx] = outletMoleFraction_;
