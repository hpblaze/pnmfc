...
file_directory = "/home/hp/dumux/harsha_pnm/build-cmake/appl/2p_magic/33micro_len_throatsource__2e-9/"
pvd_file_path = os.path.join(file_directory, "2p_33micro_len_throat_2e-9.pvd")
...
if timesteps:
    ...
    file_paths = [os.path.join(file_directory, f"2p_33micro_len_throat_2e-9-{i:05d}.vtp") for i in range(num_vtp_files)]
    ...
    # Plot p_liq against timestep for each VTP file in subplots
    for idx, pore_index in enumerate(pore_indices):
        ax = axs[idx // 2, idx % 2]  # Get subplot axis
        ax.set_xlabel('Time [s]', fontsize=12)
        ax.set_ylabel('Local p_liq [-]', fontsize=12)
        ...
        for i, p_liq_array in enumerate(p_liq_data_arrays):
            ...
            ax.plot(timesteps[i], p_liq_array[pore_index], color=color, marker=marker, markersize=6, label=f'Time Step {i}')

    # Adjust layout
    plt.tight_layout()

    # Save subplots as .png file with file names
    subplots_filename = os.path.join(file_directory, "subplots_p_liqvsRealtime.png")
...
for pore_index in pore_indices:
    # Plot p_liq against time_step for each VTP file
    ...
    for i, p_liq_array in enumerate(p_liq_data_arrays):
        ...
        marker = markers[i % len(markers)]  # Cycle through markers
        color = colors[i]  # Get color from the predefined list
        plt.plot(time_step, p_liq_array[pore_index], color=color, marker=marker, markersize=8, label=f'Time Step {i}')
    
    plt.legend(fontsize=10, loc='upper left', ncol = 4)  # Adjust legend location and font size
    
    # Save plot as .png file with file names
    plot_filename = os.path.join(file_directory, f"plot_pore_{pore_index}_p_liq_with_legend.png")
