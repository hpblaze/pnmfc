...
def extract_p_gas_from_vtp(file_path):
    try:
        ...
        p_gas_array = np.array(polydata.GetPointData().GetArray("p_napl"))
...
for idx, pore_index in enumerate(pore_indices):
    ...
    ax.set_ylabel('Local P$_n$ [-]', fontsize=12)
    ...
    for i, p_gas_array in enumerate(p_gas_data_arrays):
        ...
        ax.plot(time_step, p_gas_array[pore_index], color=color, marker=marker, markersize=6, label=f'Time Step {i}')
...
subplots_filename = os.path.join(file_directory, "subplots_p_gas_with_legend.png")
