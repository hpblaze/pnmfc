...
if timesteps:
    ...
    print(f"Number of VTP files detected: {len(transmissibilityN_data_arrays)}")
    ...
    # Plot transmissibilityN against physical time for each throat index
    for idx, throat_index in enumerate(throat_indices):
        ...
        for i, transmissibilityN_array in enumerate(transmissibilityN_data_arrays):
            ...
            ax.plot(time_step, transmissibilityN_array[throat_index], color=color, marker=markers[i % len(markers)], markersize=6, label=f'Throat {throat_index}')
