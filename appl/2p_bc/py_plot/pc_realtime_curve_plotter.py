...
if timesteps:
    ...
    # Plot pc against timestep for each VTP file in subplots
    for idx, pore_index in enumerate(pore_indices):
        ax = axs[idx // 2, idx % 2]  # Get subplot axis
        ax.set_xlabel('Time [s]', fontsize=12)
        ax.set_ylabel('Local pc [-]', fontsize=12)
        ...
        for i, pc_array in enumerate(pc_data_arrays):
            marker = markers[i % len(markers)]  # Cycle through markers
            color = colors[i]  # Get color from the predefined list
            ax.plot(timesteps[i], pc_array[pore_index], color=color, marker=marker, markersize=6, label=f'Time Step {i}')

    # Adjust layout
    plt.tight_layout()

    # Save subplots as .png file with file names
    subplots_filename = os.path.join(file_directory, "subplots_pcvsRealtime.png")
...
for pore_index in pore_indices:
    # Plot pc against time_step for each VTP file
    ...
    for i, pc_array in enumerate(pc_data_arrays):
        ...
        marker = markers[i % len(markers)]  # Cycle through markers
        color = colors[i]  # Get color from the predefined list
        plt.plot(time_step, pc_array[pore_index], color=color, marker=marker, markersize=8, label=f'Time Step {i}')
    
    plt.legend(fontsize=10, loc='upper left', ncol = 4)  # Adjust legend location and font size
    
    # Save plot as .png file with file names
    plot_filename = os.path.join(file_directory, f"plot_pore_{pore_index}_pc_with_legend.png")
