...
# Plot TransmissibilityN against time step for each VTP file in subplots
for idx, throat_index in enumerate(throat_indices):
    ...
    for i, transmissibilityN_array in enumerate(transmissibilityN_data_arrays):
        ...
        ax.plot(time_step, transmissibilityN_array[throat_index], color=color, marker=marker, markersize=6, label=f'Time Step {i}')

# Adjust layout
plt.tight_layout()

# Save subplots as .png file with file names
subplots_filename = os.path.join(file_directory, "subplots_transmissibilityN_with_legend.png")
plt.savefig(subplots_filename, bbox_inches='tight')
plt.show()
'''
# Save individual plots for each throat index with file names
for throat_index in throat_indices:
    # Plot TransmissibilityN against time_step for each VTP file
    plt.figure(figsize=(12, 8))

    plt.xlabel('Time step', fontsize=14)
    plt.ylabel('Local $g^{im}_{n,ij}$ [m$^3$/(Pa.s)]', fontsize=14)
    plt.title(f'Local TransmissibilityN vs. timestep @ throat-{throat_index}', fontsize=16)
    ...
    for i, transmissibilityN_array in enumerate(transmissibilityN_data_arrays):
        ...
        marker = markers[i % len(markers)]  # Cycle through markers
        color = colors[i]  # Get color from the predefined list
        plt.plot(time_step, transmissibilityN_array[throat_index], color=color, marker=marker, markersize=8, label=f'Time Step {i}')

    # Save plot as .png file with file names
    plot_filename = os.path.join(file_directory, f"plot_throat_{throat_index}_transmissibilityN.png")
