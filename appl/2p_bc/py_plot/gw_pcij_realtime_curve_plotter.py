...
def extract_transmissibilityW_from_vtp(file_path, throat_index):
    try:
        reader = vtk.vtkXMLPolyDataReader()
        reader.SetFileName(file_path)
        reader.Update()
        ...
        return transmissibilityW_array[throat_index]
    except Exception as e:
...
if timesteps:
    ...
    # Plot transmissibilityW against pc data for each pore index combination
    for i, (pore1, pore2) in enumerate(pore_index_combinations):
        ...
        # Get corresponding transmissibilityW data for the throat index
        transmissibilityW_array = [extract_transmissibilityW_from_vtp(file_path, throat_indices[i]) for file_path in file_paths]

        # Plot pc,ij array against transmissibilityW data with markers based on timestep
        ax = axs[i // 4, i % 4]
        for j, (pc, transmissibility) in enumerate(zip(pc_ij_array, transmissibilityW_array)):
    ...
    subplots_filename = os.path.join(file_directory, "gW_vs_pc_ij_subplot.png")
